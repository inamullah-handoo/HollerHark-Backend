    ...
    if(req.user.role != "Admin"){
        ...
        if (!errors.isEmpty()) {
            ...
            User.getUserByCategory(req.body.category, (err, users) => {
                if(err){
                    res.json({success: false, msg: 'Something wen wrong'});
                }else{
                    let newComplaint = new Complaint({
                        ...
                        log: ['[ '+ Date().substring(0,24) +' ] Registered By: ' + req.user.name]
                    });
...
router.get('/adminViewList/:status', passport.authenticate('jwt', { session: false }), (req,res) => {
    if(req.user.role == 'Admin'){
        Complaint.getComplaintByStatus(req.params.status, (err, complaints) => {
            if(err){
                ...
                if(!complaints[0]){
                    ...
                    for(i=0; i<complaints.length; i++){
                        comp[i] = {title: complaints[i].title, id: complaints[i]._id};
                    }
        ...
        if(err){
            ...
            if(!complaint){
                ...
                if(req.user._id == complaint.user_id && complaint.status == 'Not Processed Yet'){
                    const errors = validationResult(req);
                    if (!errors.isEmpty()) {
                        res.json({success: false, msg: 'Please fill all the fields', errors});
                    }else{
                        User.getUserByCategory(req.body.category, (err, users) => {
                            if(err){
                                res.json({success: false, msg: 'Something wen wrong'});
                            }else{
                                complaint.log.push('[ '+ Date().substring(0,24) +' ] Complaint Modified');
                                let editComplaint = {
                                    title: req.body.title,
                                    category: req.body.category,
                                    user_id: req.user._id,
            ...
            if(!complaint){
                ...
                if(req.user._id == complaint.dean_id){
                    ...
                    if(!errors.isEmpty()){
                        ...
                        User.getUserByID(req.body.worker_id, (err,user) => {
                            if(err) throw err;
                            if(!user){
                                res.json({success: false, msg: 'Something went wrong'});
                            }else{
                                complaint.log.push('[ '+ Date().substring(0,24) +' ] Complaint Forwarded To: ' + user.name);
            ...
            if(!complaint){
                ...
                if(req.user._id == complaint.worker_id){
                    ...
                    if(!errors.isEmpty()){
                        ...
                        complaint.log.push('[ '+ Date().substring(0,24) +' ] Response to Forwarded Complaint ');
            ...
            if(!complaint){
                ...
                if(req.user._id == complaint.dean_id){
                    ...
                    if(!errors.isEmpty()){
                        ...
                        complaint.log.push('[ '+ Date().substring(0,24) +' ] Complaint Status changed to : ' + req.body.newStatus);
            ...
            if(req.user._id == complaint.user_id){
                if(complaint.completedDate != 'Not Completed Yet'){
                    ...
                    if(!errors.isEmpty()){
                        ...
                        complaint.log.push('[ '+ Date().substring(0,24) +' ] Complaint Feedback Send');
